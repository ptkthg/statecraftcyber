...
const STYLES: Record<Variant, string> = {
  ...
  neutral:  "text-[#777] bg-white/[0.04] border-white/[0.08]",
...
export function Badge({ variant = "neutral", children, className = "" }: Props) {
  return (
    <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded border text-[10px] font-bold uppercase tracking-wider ${STYLES[variant]} ${className}`}>
      {children}
    </span>
  );
}
