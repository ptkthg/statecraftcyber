...
export default function NoticiasPage() {
  ...
  const grouped = useMemo(() => {
    ...
    return [...map.entries()]
      ...
      .map(([key, articles]) => ({ label: formatDateLabel(key), articles }));
  }, [filtered]);

  const typeCounts = useMemo(() => {
    const all = data?.articles ?? [];
    return Object.fromEntries(
      ARTICLE_TYPES.map((t) => [t, t ? all.filter((a) => a.type === t).length : all.length])
    );
  }, [data]);
    ...
    <main className="min-h-screen bg-[#050505] pt-16">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-start justify-between gap-4 flex-wrap">
            ...
            <button
              onClick={() => load(true)}
              disabled={refreshing}
              className="flex items-center gap-2 px-4 py-2 text-sm text-[#A1A1AA] hover:text-white border border-white/[0.08] hover:border-white/[0.16] rounded-lg transition-all self-start"
            >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex flex-col gap-3 mb-8">
          ...
          {/* Linha 2 — Região + stats */}
          <div className="flex items-center gap-2 flex-wrap">
            {(["", "Global", "Brasil"] as const).map((r) => {
              const count = r
                ? (data?.articles ?? []).filter((a) => a.sourceRegion === r).length
        ...
        {loading ? (
          <div className="space-y-8">
            {[...Array(3)].map((_, i) => (
              <div key={i}>
                ...
                <div className="bg-[#0D0D0D] border border-white/[0.06] rounded-xl overflow-hidden">
                  {[...Array(4)].map((_, j) => (
                    <div key={j} className="h-20 border-b border-white/[0.04] last:border-0 animate-pulse" />
                  ))}
                </div>
              </div>
            ))}
          ...
          grouped.map(({ label, articles }) => (
            <DateGroup key={label} label={label} articles={articles} />
          ))
        )}
