    ...
    <main className="min-h-screen bg-[#050505] pt-16">
      ...
      <div className="border-b border-white/[0.04] sticky top-0 bg-[#050505]/95 backdrop-blur z-10">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <Link
              href="/"
              className="flex items-center gap-1.5 text-xs text-[#555] hover:text-[#A1A1AA] transition-colors"
            >
              <ArrowLeft size={11} />
              Site
            </Link>
            <form action="/api/admin/auth" method="POST">
              <button
                ...
                className="flex items-center gap-1.5 text-xs text-[#555] hover:text-red-400 transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-red-500 rounded px-1"
              >
                <LogOut size={11} />
                Sair
              </button>
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8">
        ...
        <p className="text-xs text-[#555] mb-8">
          Dados em tempo real do banco. Atualizado a cada carregamento.
        </p>
        ...
        {lastRun && (
          <div className="bg-[#0D0D0D] border border-white/[0.06] rounded-xl p-6 mb-6">
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="text-[10px] text-[#444]">{timeAgo(lastRun.runAt)}</span>
            </div>

            <div className="grid sm:grid-cols-4 gap-4 mb-4">
              <div>
                <p className="text-[10px] text-[#555] mb-1">Status</p>
              ...
              <div>
                <p className="text-[10px] text-[#555] mb-1">Briefings criados</p>
                <p className="text-sm font-bold text-white">{lastRun.briefingsCreated}</p>
              </div>
              <div>
                <p className="text-[10px] text-[#555] mb-1">Duração</p>
                <p className="text-sm font-mono text-white">{formatDuration(lastRun.durationMs)}</p>
              </div>
              <div>
                <p className="text-[10px] text-[#555] mb-1">Erros</p>
                <p className={`text-sm font-bold ${lastRun.errors.length ? "text-red-400" : "text-[#555]"}`}>
            ...
            {lastRun.sources.length > 0 && (
              <div className="mb-3">
                <p className="text-[10px] text-[#555] mb-1.5">Fontes consultadas</p>
            ...
            {lastRun.errors.length > 0 && (
              <div>
                <p className="text-[10px] text-[#555] mb-1.5">Erros</p>
        ...
        <div className="bg-[#0D0D0D] border border-white/[0.06] rounded-xl overflow-hidden">
          ...
          <div className="hidden sm:grid grid-cols-[auto_1fr_auto_auto_auto_auto] gap-4 px-5 py-2 border-b border-white/[0.04] bg-white/[0.01]">
            {["Status", "Horário", "Briefings", "Duração", "Fontes", "Erros"].map((h) => (
              <span key={h} className="text-[9px] font-bold text-[#555] uppercase tracking-wider">{h}</span>
            ))}
          </div>

          {cronLogs.length === 0 ? (
            <p className="px-5 py-8 text-sm text-[#555] text-center">Nenhuma execução registrada ainda.</p>
          ) : (
            cronLogs.map((log) => (
              <div
                key={log.id}
              ...
              >
                ...
                <span className="text-[11px] text-[#555]">{log.sources.length}</span>
                <span className={`text-[11px] font-bold ${log.errors.length ? "text-red-400" : "text-[#444]"}`}>
                  {log.errors.length || "—"}
                </span>
              </div>
            ))
        ...
        <div className="mt-6 flex items-center gap-4">
          <Link
            href="/api/health"
            target="_blank"
            className="text-xs text-[#555] hover:text-[#A1A1AA] transition-colors font-mono"
          >
            /api/health ↗
          </Link>
        </div>
